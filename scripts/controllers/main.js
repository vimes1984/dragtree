...
angular.module('timerApp')
  .controller('MainCtrl', function ($scope, $document) {
	...
	$scope.timerConsole = [];
    $scope.timerType = '';
  	$scope.graph = {'width': 100, 'height': 134};
  	$scope.reactiontimes = [];
  	//light colors pre go
    ...
    $scope.startTimer = function (){
    	...
    	$scope.amberstrtscnd =  $scope.randstart + 500;
    	$scope.amberstrtthird = $scope.randstart + 1000;
    	$scope.agreenstrt = $scope.randstart + 1500;
    ...
    $scope.$on('timer-tick', function (event, args) {
              //  $scope.timerConsole.push({'"'+$scope.timerType +'":'+ event.name + ', "timeoutId":' + args.timeoutId + ', "millis":' + args.millis};
               ...
               $scope.endarray = $scope.timerConsole[$scope.timerConsole.length -1]; 
				...
				if($scope.endarray.millis >= $scope.amberstrtscnd){
					$scope.amberpalesecond = 'rgba(255, 228, 27, 1)';
					$scope.$apply();
				}
				if($scope.endarray.millis >= $scope.amberstrtthird ){
...
After the two top lights go on (the pre-stage light and the staging light), there is a half second delay between the amber lights and between the last amber light and the green light.
After the pre-stage and the staging light, there should be about 3 seconds delay before the amber light start.  
This can be anywhere from 1-3 seconds but not more than 3 seconds.  This can be random.
